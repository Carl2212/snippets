...
@Component({
  ...
  template : `
    <div class="drag-box" #dragBox>
     <span #scaleTop class="point top" tappable></span>
     <span #scaleRight class="point right" tappable></span>
     <span #scaleBottom class="point bottom" tappable></span>
     <span #scaleLeft class="point left" tappable></span>
...
export class DragBox {
  ...
  isScalePoint(x: number, y: number, x1: number, y1: number) {
    let gap = Math.sqrt(Math.pow((x - x1),2) + Math.pow((y - y1),2));
    if(gap < 16) {
      return true;
    }
    return false;
